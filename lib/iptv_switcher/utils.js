...
const loadFromFile = async (filePath) => {
  ...
  await Promise.all(iptv.segments.segments.map(async (channel) => {
    const tokenizedUrl = await GET(`http://streambay.duckdns.org/iptv?url=${channel.url}`);
    channel.url = tokenizedUrl.url;
  }));
};
